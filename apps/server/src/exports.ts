...
const exportStyles = `
@font-face{font-family:Handbook;src:url('http://render.local/font.otf')}*{box-sizing:border-box}body{margin:0;background:#f8f5ed;color:#49443d;font-family:Handbook,sans-serif}.sheet{width:1080px;padding:64px;background:radial-gradient(#ddd5c5 1px,transparent 1px) 0 0/22px 22px,#f8f5ed}.masthead{display:flex;justify-content:space-between;align-items:center;border-bottom:2px solid #dcd5c7;padding-bottom:32px;margin-bottom:36px}.kicker{font-size:19px;letter-spacing:4px;color:#a38f76}h1{font-size:52px;letter-spacing:3px;margin:10px 0 14px}h2{font-size:26px;font-weight:400;margin:0}.date{font-size:28px;color:#8d7960}.moment{margin:0 0 30px;display:flex;gap:26px;align-items:flex-start;break-inside:avoid}.time{font-size:24px;color:#9b8b73;width:96px;padding-top:30px;flex-shrink:0}.card{flex:1;min-width:0;padding:30px;background:#fffdf9;border:2px solid #e7e0d3;border-radius:28px;box-shadow:0 6px 0 #e9e1d54d}.person{display:flex;align-items:center;gap:14px;font-size:26px;margin-bottom:24px}.dot{width:18px;height:18px;border-radius:50%}.media{width:100%;height:auto;max-height:1300px;object-fit:contain;display:block;border-radius:16px}.sticker{width:260px;height:260px;object-fit:contain;display:block;margin:10px auto 28px}.description{font-size:28px;line-height:1.8;white-space:pre-wrap;overflow-wrap:anywhere;margin:22px 0 0}.credit{font-size:14px;color:#9f9588;margin:20px 0 0}.footer{font-size:18px;text-align:center;color:#a89c89;padding-top:18px;letter-spacing:2px}`;
...
function template(
  ...
  return `<!doctype html><html lang="zh-CN"><meta charset="utf-8"><style>${exportStyles}</style><body><main class="sheet"><header class="masthead"><div><div class="kicker">FRIENDS IN PARALLEL</div><h1>此刻，同频</h1><h2>不同的小日常，同一天的我们。</h2></div><div class="date">${date}<br><small>${new Set(items.map((i) => i.entry.personId)).size} 位朋友 · ${items.length} 个瞬间</small></div></header>${indices.map((i) => article(items[i], i)).join('')}<footer class="footer">把普通的一天，收进我们的手账。 · ${page} / ${total}</footer></main></body></html>`;
}
