...
选择「下载素材 ZIP」后先进入提示词页，可选择生成图片或视频、填写风格与期望，并一键复制完整提示词。提示词说明 `manifest.json` / `manifest.csv`、朋友素材目录、北京时间、描述、素材路径和许可的含义与用法。随后点击「下载压缩包」，将 ZIP 和提示词一起交给 AI；无需先复制也可直接下载，浏览器不允许自动复制时可手动选择复制。

长图、长图合集和素材 ZIP 均使用带动态日期、类型及北京时间导出日期的文件名，例如 `此刻同频_2026-08-30_手账-01_导出2026-09-07.png`、`此刻同频_2026-08-30_素材包_导出2026-09-07.zip`；分页长图带两位页码，合集内的图片也使用该命名规则。
