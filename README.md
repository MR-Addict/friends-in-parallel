...
首页「制作回忆」入口可将所选日期的动态生成手账长图、回忆视频，或下载素材 ZIP。单张图片预览和视频结果支持系统分享，按资源类型检测 `navigator.canShare`。动态操作菜单和 ZIP 不提供分享入口；ZIP 仍可下载。视频结果使用「分享视频」主按钮和「修改样式与音乐」次按钮，不再提供视频下载按钮；不支持文件分享的浏览器会隐藏分享按钮。
