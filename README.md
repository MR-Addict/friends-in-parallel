...
视频完整包含当天所有朋友，按北京时间从早到晚播放：日期片头（2 秒）、小时章节（1.5 秒）、逐条动态、署名片尾（至少 5 秒）。照片保持完整，文字按实际排版高度拆成续页，不截断、不自动摘要。每页停留 `max(4, ceil(字符数 / 6) + 1)` 秒，转场另计 0.5 秒（夜色样式 1 秒），因此内容较多的视频也较长。背景音乐循环或裁剪至成片长度，统一响度后首尾淡入淡出；无音乐视频不包含音轨。
